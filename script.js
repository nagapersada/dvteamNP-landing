...
function loadMemberData() {
    ...
    // PERBAIKAN: Ubah jadi huruf kecil semua agar tidak error (DEKA -> deka)
    if (refCode) {
    ...
    // PERBAIKAN FOTO: Tambahkan timestamp (?v=...) untuk memaksa foto baru dimuat
    const imgElement = document.getElementById('member-photo');
    if(imgElement) {
        // Trik Anti-Cache: Tambahkan waktu sekarang di belakang nama file
        imgElement.src = data.photo + "?v=" + new Date().getTime();
        
        // Jika foto error (tidak ada), ganti logo
        imgElement.onerror = function() { 
            if(!this.src.includes('logo.png')) this.src = 'logo.png'; 
        };
    }

    // Update Link WA
    const message = `Halo Kak ${data.name}, saya tertarik info bisnis DV NP.`;
    const waButton = document.getElementById('cta-wa');
    if(waButton) {
        waButton.href = `https://wa.me/${data.wa}?text=${encodeURIComponent(message)}`;
    }
...
// Jalankan saat halaman loading (Hanya jika bukan di landing page yang punya script sendiri)
